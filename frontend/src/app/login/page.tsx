...
function LoginForm() {
  ...
  async function handleEmailAuth(e: FormEvent) {
    ...
    if (mode === "signup") {
      const { error } = await supabase.auth.signUp({
        email,
        password,
        options: { emailRedirectTo: `${location.origin}/auth/callback?next=${from}` },
      });
  ...
  async function handleGoogle() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${location.origin}/auth/callback?next=${from}` },
    });
  ...
  return (
    <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-8 space-y-5">
      ...
      <p className="text-center text-xs text-neutral-500">
        {mode === "login" ? (
          <>No account?{" "}
            <button onClick={() => { setMode("signup"); setError(null); setSuccess(null); }} className="text-blue-400 hover:text-blue-300 transition">Sign up</button>
          </>
        ) : (
          <>Already have an account?{" "}
            <button onClick={() => { setMode("login"); setError(null); setSuccess(null); }} className="text-blue-400 hover:text-blue-300 transition">Sign in</button>
          </>
        )}
      </p>
    </div>
    ...
    <main className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="mb-10 text-center">
          ...
          <p className="mt-1 text-xs text-neutral-600 uppercase tracking-widest">Sign in to your account</p>
        </div>
        <Suspense fallback={<div className="h-64 rounded-2xl border border-neutral-800 bg-neutral-900" />}>
          <LoginForm />
        </Suspense>
